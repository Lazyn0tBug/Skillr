---
date: 2026-04-21
topic: skilr-skill-routing-framework
---
...
**Core Commands**
- R1. `/SkilrScan` — 扫描用户配置的 Skills 目录，生成 `skilr_index.json`（机器可读）和 `SKILR_INDEX.md`（人类可读）
- R2. `/Skilr <用户任务>` — 分析用户意图，匹配可用 Skills，返回候选命令列表
...
**Index Management**
- R6. 安装后执行首次扫描，之后每次 `/Skilr` 调用时检查 Skills 目录 mtime，懒重建 stale 索引
...
- SC1. 用户在 Claude Code 中调用 `/SkilrScan` 能成功扫描配置的 Skills 目录并生成索引
- SC2. 用户调用 `/Skilr 我想做一个用户认证系统` 能返回匹配的命令建议列表
...
- Skillr Plugin 只管理自己的 Skill（`/SkilrScan`、`/Skilr`），无法直接扫描 Plugin 外部目录，但通过 `userConfig` 配置后拥有扫描权限

## Key Decisions

- **两个独立 Skill**：`/SkilrScan` 和 `/Skilr` 分别注册为独立 Skill（各自 `skills/skilrscan/SKILL.md` 和 `skills/skilr/SKILL.md`）。职责分离清晰，用户可以单独刷新索引而不触发路由。
...
- **懒重建索引**：每次 `/Skilr` 调用时检查 mtime，必要时才重建。避免维护守护进程或依赖外部调度。
...
- [Technical] **索引存储位置**：`skilr_index.json` 和 `SKILR_INDEX.md` 存放在 plugin data 目录（`${CLAUDE_PLUGIN_DATA}`）还是用户工作目录
