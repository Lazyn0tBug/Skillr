...
**存储层（按需加载）：**
- ChromaDB 和 fastembed ONNX 仅在 `embedding_backend = "model"` 时加载
- `claude` 模式（默认）：不加载任何向量组件，零额外依赖
- `model` 模式：`embedding_backend = "model"` 时初始化 ChromaDB + fastembed
...
**边界：**
- `"model"` 模式：向量搜索失败时 → 降级回关键词过滤，并记录 debug 日志
- `"claude"` 模式：无需降级，LLM 本身就能做语义理解
- E3 不会完全替代 LLM Ranking（混合模式）

**成功标准：**
- `model` 模式 gated on recall@5 评估达成（见下方评估计划）
- `claude` 模式为主流默认，无需任何额外依赖

**Recall@5 评估计划（`model` 模式启动条件）：**
1. 建立基准：纯关键词过滤在 20+ 测试 query 上的 recall@5
2. 向量匹配：相同测试集上测量 recall@5 提升
3. 目标：提升 >= 20% 才启动 `model` 模式
4. 测试 query 来源：从真实用户路由日志采样（E5 历史数据）
...
- Rust 理论预估 1000 skills: ~15-30ms（纯 Rust，二进制独立运行）
- subprocess 调用开销: ~5-15ms/次（fork+exec+序列化）

**Rust 价值的正确理解：**
- 不是「比 Python 快多少倍」——subprocess 往返后实际提速有限
- 核心价值是「**透明内嵌**」：`/skillr` 路由时发现 index 过期，直接调用 Rust scan（<50ms），用户无感知，无需手动 scan
- 消除「scan 之后文件变了但 index 还是旧的」状态不一致问题

**解决的具体场景：**
1. 用户新增一个 skill 后，立即运行 `/skillr`（无需先跑 `/skillscan`）
2. 自动化工具安装新 skill 后，下次路由自动感知
3. 索引损坏或丢失时，`/skillr` 自动重建而非报错
...
│  │  history.py   — SelectionHistory (DuckDB)            │   │
│  │  config.py    — 只读访问                             │   │
│  └─────────────────────────────────────────────────────┘   │
│                          │                                  │
│                          ▼                                  │
...
| IntentCache 读写 | `cache.py` | `skillr-core cache`（需 HMAC 签名写入） |
| Config 读写 | `config.py` | `skillr-core config` |
| SelectionHistory | `history.py` | **留在 Python**（DuckDB Python SDK） |
...
- SelectionHistory（`history.py`）— DuckDB Python SDK，E5 历史数据
...
# 缓存操作（Rust 读写 JSON 文件，需 HMAC 签名）
skillr-core cache --get "intent_hash"
skillr-core cache --set "intent_hash" '["match_results_json"]' --signature "<hmac_sha256_hex>"

# Config 读写
skillr-core config --get embedding_backend
skillr-core config --set embedding_backend "model"
```

> ⚠️ `cache --set` 必须包含 HMAC-SHA256 签名。签名 = `HMAC-SHA256(entries_json, cache_secret)`，其中 `cache_secret` 读取自 `${CLAUDE_PLUGIN_DATA}/config.json`。无签名或签名无效的写入会被 Python 层在读取时丢弃。

**SelectionHistory 保持在 Python 层**（E5 历史使用 DuckDB Python SDK，Rust 不负责）。
...
**Rust CLI 安全要求（来自代码 review）：**
> ⚠️ 以下安全不变量必须在 Rust 实现中遵循，否则会导致与 Python 层行为不一致或安全漏洞。详见 [Security Fixes Applied](#security-fixes-applied-post-review)。

| 不变量 | 要求 | 对应问题 |
|--------|------|---------|
| **原子写入** | `skillr-core scan` 写 index 时必须用 `std::fs::rename()`（POSIX rename 是原子 syscall）。禁止 read-modify-write 序列。 | ADV-001 |
| **Cache 签名写入** | 写 `intent_cache.json` 前必须用 HMAC-SHA256(entries_json, `cache_secret`) 签名。`cache_secret` 读取自 `${CLAUDE_PLUGIN_DATA}/config.json`。 | ADV-007 |
| **Cache 签名验证** | 读 `intent_cache.json` 时验证 HMAC 签名，签名无效时丢弃 cache（Python 层会重建）。 | ADV-007 |
| **索引重建后清除 Cache** | `skillr-core scan` 完成后，Python wrapper 必须调用 `cache.invalidate_all()`。Rust 本身不管理 intent cache 的失效逻辑。 | ADV-006 |
| **无 DuckDB 操作** | Rust 二进制不包含 DuckDB 代码。SelectionHistory 写入由 Python 层 `history.py` 处理。 | ADV-002 |
...
**E2 与 E4 的边界（重要）：**
- **E2（增量索引）**：解决「scan 变快了」问题——只扫描有变化的文件，scan 时间 < 全量 20%
- **E4（Rust Core）**：解决「scan 可有可无」问题——全量 scan 快到可以在路由时透明执行，无需用户手动
- 两者互补，不重叠：E4 让「scan 命令」变成可选的预热优化，E2 让显式 scan 更快
...
├── cache.py         # 保留：HMAC 签名/验证（router.py 直接调用）
├── history.py       # 保留：SelectionHistory（DuckDB Python SDK）
└── (scanner.py, indexer.py, config.py)
    → 迁移到 Rust，不在 Python 层存在
```

> ⚠️ `cache.py` 保留在 Python 层（不迁移），原因：
> - HMAC 签名/验证依赖 Python 层的 `cache_secret`
> - `router.py` 直接调用 `cache.py` 的 `get()`/`set()`/`invalidate_all()`
> - Rust 的 `skillr-core cache` 子命令仅处理文件 I/O，HMAC 逻辑由 Python 层完成
> - 见 [ADV-007](#security-fixes-applied-post-review) 详解
...
- ~~E2（增量索引）：如何检测 skills_dir 本身被删除或重建？~~ ✅ 已解决：`build_incremental_index()` 在遍历 `get_skills_dirs()` 时，如果 dir 已删除，`scan_skills_dir` 返回 `([], {})`，旧 tracking 数据不再被复用，新的 index 中该 dir 的 skills 为空。

## Dependencies

- E1 依赖 E2 的变化检测来实现缓存失效（可独立开发，但运行时有依赖）
  > ⚠️ 更新：E1 现在调用 `invalidate_all()`，不依赖 E2 的变化检测。任何 index 重建（full 或 incremental）都会清除所有缓存。
- E3（`model` 模式）依赖 E1（缓存基础设施）；E3（`claude` 模式）无额外依赖
- E4（Rust Core）可在任何阶段独立进行；**但必须遵循 [Rust CLI 安全要求](#rust-cli-安全要求来自代码-review)，否则与 Python 层 cache HMAC 验证不兼容**
- E5 Phase 1.5（时间窗口展示）不依赖 E1/E2/E3，可独立进行
- E5 Phase 2（Ranking 加权）依赖 Phase 1.5 完成后的 DuckDB 基础设施
- E5 历史查询（Python → Rust）：**已确定留在 Python 层**，Rust 不处理 DuckDB
...
### ADV-001: Cache Atomic Write Vulnerability ✅ Fixed

**Problem**: If process is killed between `fsync()` and `rename()`, orphaned `.tmp` file causes cache to be permanently emptied on next load.

**Python Fix**: `_load_cache()` now checks for orphaned `.tmp` on startup:
- If `.tmp` exists but `.json` doesn't → remove orphaned `.tmp`
- If both exist → `.json` is likely valid, remove `.tmp` only

**Rust CLI Requirement**: Must use `std::fs::rename()` for atomic replace. `rename()` on POSIX is atomic (as rename syscall). Do NOT use multi-step read-modify-write.

### ADV-002: DuckDB Lock Silent Data Loss ✅ Fixed

**Problem**: DuckDB file lock causes `add_record()` to silently fail (selection history permanently lost).

**Python Fix**: `add_record()`, `get_skill_selection_count()`, `get_skill_stats()` now log `logger.warning()` on exception before silently returning. Operators can observe persistent failures in logs.

**Rust CLI Requirement**: N/A — history stays in Python (DuckDB Python SDK).

### ADV-006: Cache Coherence (Skill Content Change) ✅ Fixed

**Problem**: `skill_ids_hash` only tracks skill **names**, not content. If a skill's description changes (same name), cache key is unchanged and stale `MatchResult` served.

**Python Fix**: `run_indexer()` now calls `cache.invalidate_all()` unconditionally after any index rebuild. All cached results are invalidated since we cannot cheaply detect content-only changes.

**Rust CLI Requirement**: When `skillr-core scan` is invoked (from `/skillscan` or transparent auto-rebuild in E4), the Python wrapper must call `cache.invalidate_all()` after `run_indexer()` returns.

### ADV-007: Cross-User Cache Poisoning ✅ Fixed

**Problem**: Shared `intent_cache.json` allows one user to pre-populate entries that favor malicious skills for known intent strings.

**Python Fix**: Cache entries are signed with HMAC-SHA256 using a machine-specific secret stored in `${CLAUDE_PLUGIN_DATA}/config.json` (`cache_secret`). On load, signature is verified. Tampering → cache discarded. Pre-existing caches (no signature) are trusted on read and re-signed on write.

**Rust CLI Requirement**: When `skillr-core` writes to the cache (future E4 integration), it must:
1. Read the `cache_secret` from `${CLAUDE_PLUGIN_DATA}/config.json`
2. Sign entries with `HMAC-SHA256(entries_json, cache_secret)` before writing
3. On read, verify signature — discard if invalid

The secret is shared between Python and Rust (same `config.json`), ensuring consistency.
