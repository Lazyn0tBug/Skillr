...
- 局限性：查询「某 skill 被选多少次」需扫描全文件；时间窗口过滤代价高

---

#### Phase 1.5 — 时间窗口 + 展示选择次数
...
**预计改动文件：**
- `src/skillr/history.py`（重写为 DuckDB）
- `src/skillr/router.py`（加展示逻辑）
- `tests/test_history.py`（改造）
- `skills/skillr/SKILL.md`（更新示例）
...
- E5 Phase 2（Ranking 加权）依赖 Phase 1.5 完成后的 SQLite 基础设施
