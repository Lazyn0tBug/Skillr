...
| 步骤 | Skilr 实现方式 | 交付物 |
|:---|:---|:---|
| 1. 形态为 Skill，含 `/SkilrScan` 和 `/Skilr` | 作为宿主 Agent 的插件/Skill 注册，暴露两个 Slash 命令 | `skill.yaml` + 命令路由入口 |
| 2. `/SkilrScan` 扫描生成本地 Skills 概览 | 遍历 `./skills/`，解析元数据，输出 `SKILR_INDEX.md`（人类可读）+ `skilr_index.json`（机器可读） | 静态索引文件 |
| 3. `/Skilr <任务>` 分析用户意图 | 调用一次轻量 LLM，输出结构化 `task_spec.json`（意图/约束/关键词） | 意图压缩结果 |
...
#### 2. `skilr_index.json`（`/SkilrScan` 输出）
```json
[
  {
    "id": "code_gen",
...
def scan_skills(skills_dir: str = "./skills"):
    ...
    save_json(index, "skilr_index.json")
    save_markdown(index, "SKILR_INDEX.md")  # 生成人类可读概览
    return f"✅ 扫描完成，共发现 {len(index)} 个 Skills。索引已更新至 skilr_index.json"
```
...
def route_and_select(user_prompt: str):
    ...
    # 2. 匹配 Skills
    index = load_json("skilr_index.json")
...
```
skilr/
├── skill.yaml              # Skilr 自身元数据
├── commands/
│   ├── skilrscan.py        # /SkilrScan 实现
│   └── skilr.py            # /Skilr 实现
├── core/
...
│   └── test_runner/
│       └── skill.yaml
├── skilr_index.json        # /SkilrScan 生成
├── SKILR_INDEX.md          # /SkilrScan 生成
└── README.md
```
...
2. **实现 `/SkilrScan`**（遍历目录 → 生成索引文件）
3. **实现 `/Skilr` 核心流**（意图提取 → 匹配 → 选择 → 组装）
